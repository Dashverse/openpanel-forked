              ...
              if (payload) {
                ...
                if (duplicate) {
                  ...
                  try {
                    ...
                    // SDK/producer retries land seconds later so the window is
                    // tiny, and the CH event `id = $insert_id ?? uuid` backstop
                    // in event.service.ts still dedups via ReplacingMergeTree.
                    // Not marked on a handler throw either, so a retry can
                    // re-attempt.
                    if (dedupKey) {
                      getRedisEvent()
