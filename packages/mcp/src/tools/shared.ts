...
/**
 * Reusable filter schema for analytics tools. AND-combined. Property naming
 * follows the same convention as breakdowns: a bare name for a built-in column
 * (`country`, `os`, `device`, `path`), or `properties.<key>` for a custom event
 * property (`properties.gateway`). Call `get_property_values` to discover valid
 * names and values. The engine automatically uses a materialized column when
 * one exists, else falls back to the properties map.
 */
export const zFilters = z
  .array(
    z.object({
      name: z
        .string()
      ...
      operator: z
        ...
        .describe('How to match. Use isNull/isNotNull with no value.'),
      value: z
