...
export async function getChartSql({
  ...
  // Event-level property filters/breakdowns are served by the events path below,
  // which resolves each property to its MATERIALIZED column via getSelectPropertyKey
  // (anon-inclusive, fast on materialized keys).
